...
export const ProviderDetails = ({
        ...
        const loadProviderData = async () => {
            try {
                ...
                try {
                    ...
                    // Don't set error state for availability issues
                }
    ...
    const handleSaveProvider = async () => {
        try {
            const currentProviderId = providerId || provider?._id;
            console.log('Saving provider:', currentProviderId);
            
            await saveProvider(currentProviderId);
    ...
    const handleRemoveSavedProvider = async () => {
        try {
            const currentProviderId = providerId || provider?._id;
            console.log('Removing saved provider:', currentProviderId);
            
            // Find the saved provider entry
            const savedProvider = savedProviders.find(sp => sp.provider.id === currentProviderId);
            
            if (savedProvider) {
                await removeSavedProvider(savedProvider.savedId);
    ...
    if (loading) {
        return (
            <div className={`${isModal ? "p-8" : "min-h-screen"} flex justify-center items-center bg-alice_blue-500`}>
    ...
    const content = (
        <div className={`${isModal ? "divide-y divide-alice_blue-200" : "space-y-6 pt-28 pb-16"}`}>
            {successMessage && (
                <div className={`${theme.status.success} p-4 rounded-lg mb-4 transition-opacity duration-500`}>
                    {successMessage}
                </div>
            )}
                ...
                <div className="flex flex-col md:flex-row items-start gap-6">
                    ...
                    <div className="flex-grow space-y-4">
                        <div>
                            ...
                            <p className="text-prussian_blue-400 mb-2">{provider?.credentials}</p>
            ...
            <div className={`${isModal ? "py-6" : `${theme.card.default} p-6`}`}>
                ...
                <p className="text-prussian_blue-400 leading-relaxed">{provider?.bio}</p>
            </div>
    ...
    // Render modal version if isModal is true
    if (isModal) {
        return (
            <div
